...
# ─── Parsing ─────────────────────────────────────────────────────────────
def classify_office(raw_type: str, province: str,
                    district: Optional[str]) -> Optional[OfficeType]:
    ...
    if key in {"house", "senate"} and district:
        if re.search(r"\bCD\b|^[A-Z]{2}-\d+$", district):
            return OfficeType.US_HOUSE if "house" in key else OfficeType.US_SENATE
        if re.search(r"\bHD\b|\bSD\b", district):
            return OfficeType.STATE_HOUSE if "HD" in district else OfficeType.STATE_SENATE
    return mapped
...
def normalize_district(raw: Optional[str], office: OfficeType) -> Optional[str]:
    ...
    if not raw or office in {OfficeType.US_SENATE, OfficeType.GOVERNOR,
        ...
        return None
    m = DISTRICT_NUMBER_RE.search(raw)
    if not m:
        return None
    num = m.group(1)
...
def to_schema(race_json: dict, *, allow_offices: set[OfficeType],
    ...
    office = classify_office(raw_type, province, district_raw)
    ...
    district = normalize_district(district_raw, office)
